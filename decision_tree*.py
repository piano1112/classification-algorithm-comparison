...
MIN_SAMPLES = 77       # minimum number of examples to allow a split
ATTRIBUTE_NAMES = ['tp', 'gc', 'bp', 'sft', 'si', 'bmi', 'dpf', 'age']
...
def dt_to_string(node, depth=0):
    ...
    # internal node
    lines = []
    attr_name = ATTRIBUTE_NAMES[node.attr_index]
    for value, child in node.children.items():
        ...
        if child.children:
            ...
            # child is leaf node
            lines.append(f"{prefix}{attr_name} = {value}: {child.cl}\n")
    return ''.join(lines)
...
def classify_dtstar(training_filename, testing_filename):
...
# Test
classify_dtstar('pima-discrete.csv', 'pima-discrete.csv')
